...
class TestCompaniesWithoutParams:
    ...
    def test_companies_base_status_headers_schema(self, response_get_base_companies):
        """Проверяет базовые параметры ответа: статус-код, хедеры, соответствие json-схеме"""
        tester = BaseStatusHeadersSchemaTests(response_get_base_companies, json_schemas.COMPANIES_MAIN, 200)
    ...
    def test_companies_default_limit(
            self, response_get_base_companies):
        """Проверяет лимит по умолчанию (равен 3)"""
        body = response_get_base_companies.json()
...
class TestCompaniesRedirectFromHTTP:
    ...
    def test_companies_response_headers_with_http(
        ...
        check.is_in(response_companies_with_http.headers['Location'],
                    ('https://send-request.me/api/companies',
                     'https://send-request.me/api/companies/'),
                    f"Перенаправление на неверный адрес: {response_companies_with_http.headers['Location']}")
        check.equal(response_companies_with_http.headers['Connection'], 'keep-alive',
...
class TestCompaniesWithOffset:
    ...
    @allure.severity(Severity.NORMAL)
    def test_companies_with_valid_offset_within_range(self, api_client_companies: APIClient):
        """
    ...
    @allure.severity(Severity.NORMAL)
    def test_companies_with_valid_offset_out_of_range(self, api_client_companies: APIClient):
        """
...
class TestCompaniesWithLimitAndOffset:
    ...
    def test_companies_with_valid_limit_and_offset(self, api_client_companies: APIClient):
...
class TestCompaniesWithStatus:
    ...
    def test_companies_filtered_by_valid_status(self, status: str, expected: str,
                                                api_client_companies: APIClient, companies_grouped_by_statuses: dict):
        """
        ...
        check.equal(len(data), len(companies_grouped_by_statuses[status]),
                    f"Количество компаний в ответе ({len(data)}) не соответствует "
                    f"ожидаемому ({companies_grouped_by_statuses[status]})")
